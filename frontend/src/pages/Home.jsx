...
const Home = () => {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            // Navigate to Result page with the evaluation data
            navigate('/result', { state: { evaluation: res.data.evaluation, idea: formData } });
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="max-w-4xl mx-auto pb-20">
            ...
            <div className="glass-card rounded-2xl p-8 animate-slide-up max-w-2xl mx-auto">
                <form onSubmit={onSubmit} className="space-y-6">
                    <div>
                        <label className="block text-sm font-medium mb-2 text-slate-300">Startup Title</label>
                        <input
                            type="text"
                            name="title"
                            value={title}
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-2 text-slate-300">Elevator Pitch</label>
                        <textarea
                            name="description"
                            value={description}
                            onChange={onChange}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-2 text-slate-300">Revenue Model</label>
                            <input
                                type="text"
                                name="revenueModel"
                                value={revenueModel}
                    ...
                    >
                        {loading ? (
                            ...
                            <>
                                <Sparkles className="w-5 h-5 group-hover:animate-pulse" /> Evaluate Idea <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                            </>
                        )}
                    </button>
                </form>
